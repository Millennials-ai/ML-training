...
def _parallel_build_trees(tree, forest, X, y):
	if forest.sampling is None:
		...
		sampler = RandomOverSampler(random_state=tree.random_state)
	elif forest.sampling == 'down':
		sampler = RandomUnderSampler(random_state=tree.random_state)
...
class ResampleForestClassifier(BaseEstimator, MetaEstimatorMixin):
	""" Docstring for ResampleForestClassifier
		...
		Sampling has three valid options:
		None will invoke the standard BootstratSampler
		'up' will invoke the RandomOverSampler
		'down' will invoke the RandomUnderSampler
